...
export const getData = async (req: Request, res: Response) => {
  ...
  try {
    await LinkedInService.extractData(urn);
    res.status(200).send({ message: 'Data successfully received and stored in the database' });
  } catch (error) {
...
export const storeCookies = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).send({ message: 'Cookie successfully received and stored in the database' });
  } catch (error) {
