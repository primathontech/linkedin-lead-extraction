...
const extractDetailsForCSV = (item: any) => {
  ...
  const profileUrl = profileId ? `https://www.linkedin.com/in/${profileId}` : '';

  const companyIdMatch = currentPosition?.companyUrn?.match(/fs_salesCompany:(\d+)/);
  const companyId = companyIdMatch ? companyIdMatch[1] : '';
  const companyUrl = companyId ? `https://www.linkedin.com/company/${companyId}` : '';
  ...
  return {
    ...
    tenureAtCompany: convertMonthsToYearsMonths(currentPosition.tenureAtCompany?.numMonths || 0),
    startedOn: currentPosition.startedOn ? `${currentPosition.startedOn.month}/${currentPosition.startedOn.year}` : '',
    title: currentPosition.title || '',
    tenureAtPosition: convertMonthsToYearsMonths(currentPosition.tenureAtPosition?.numMonths || 0),
...
export const convertArrayToCSV = (data: any[]) => {
  const headers = [
    'firstName', 'lastName', 'fullName', 'geoRegion', 'currentPosition', 'companyName',
    'tenureAtCompany', 'startedOn', 'title', 'tenureAtPosition', 'companyIndustry',
    'companyLocation', 'companyUrl', 'profileUrl'
  ];
...
export const downloadCsvFile = (data: any[], fileName = `lead-data-${Date.now()}.csv`) => {
  try {
...
export const extractParameters = ({ url, sessionId, start }: { url: string, sessionId: string, start: number }) => {
...
export const convertArrayToCSVCompanySearch = (data: any[]) => {
  return data.map(item => ({
    ...
    companyUrl: item.entityUrn ? `https://www.linkedin.com/sales/company/${item.entityUrn.split(":").pop()}` : ''
  }));
};

// Download CSV file for Company Search
export const downloadCsvFileCompanySearch = (data: any[], fileName = `company-data-${Date.now()}.csv`) => {
  try {
    const ws = fs.createWriteStream(fileName);
